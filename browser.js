...
function listenForStream(page) {
    return new Promise(async (resolve, reject) => {
        ...
        const onResponse = (event) => {
            if (event.response.url.includes("https://contribution.usercontent.google.com/download?c=" && event.response.status === 200 )) { //Response should be 200
...
async function run(promptArray) {
    ...
    for (let i = 0; i < promptArray.length; i++) {
        ...
        try {
            ...
            console.log(base64Video); //focus on this line
